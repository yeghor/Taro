...
const Switcher = (props: LangSwitcherProps) => {
    ...
    return (
        ...
        <span className={`absolute left-3 text-xs font-bold text-white/60 transition-opacity duration-300 ${currentLang == "ENG" ? 'opacity-100' : 'opacity-0'}`}>
            UA
        </span>
        <span className={`absolute right-3 text-xs font-bold text-white/60 transition-opacity duration-300 ${currentLang == "ENG" ? 'opacity-100' : 'opacity-0'}`}>
            EN
        </span>
